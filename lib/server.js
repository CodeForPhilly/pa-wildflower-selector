...
module.exports = async function ({
  ...
  app.post("/get-vendors", async (req, res) => {
    try {
      ...
      const offerings = await nurseryOfferings.find(
        ...
        { projection: { plantagentsVendorId: 1, productUrl: 1, productUrlType: 1 } }
      ).toArray();
      if (!offerings.length) return res.json([]);
      const offeringByVendor = new Map(offerings.map((row) => [row.plantagentsVendorId, row]));
      const vendorIds = [...offeringByVendor.keys()];
      ...
      return res.json(vendors.map((vendor) => ({
        ...vendorStoreResult(vendor, offeringByVendor.get(vendor.plantagentsVendorId), {
  ...
  app.post("/get-online-vendors", async (req, res) => {
    try {
      ...
      if (!state) return;
      const offerings = await nurseryOfferings.find(
        { snapshotId: state.activeSnapshotId, localPlantId: plantName },
        { projection: { plantagentsVendorId: 1, productUrl: 1, productUrlType: 1 } }
      ).toArray();
      if (!offerings.length) return res.json([]);
      const offeringByVendor = new Map(offerings.map((row) => [row.plantagentsVendorId, row]));
      const vendors = await nurseries.find({
        snapshotId: state.activeSnapshotId,
        salesModel: "mail_order",
        plantagentsVendorId: { $in: [...offeringByVendor.keys()] },
      }).sort({ name: 1 }).toArray();
      return res.json(vendors
        .map((vendor) => onlineVendorResult(
          vendor,
          offeringByVendor.get(vendor.plantagentsVendorId),
          plantName
        ))
        .filter((vendor) => vendor.storeUrl && vendor.isDirectPlantLink));
    } catch (error) {
      console.error("Error in /get-online-vendors:", error.message);
      return res.status(500).json({ error: "Could not find online vendors" });
    }
  });
  ...
  app.get("/api/v1/plants", async (req, res) => {
    try {
      ...
      // Height is stored as display text plus a normalized numeric array.
      const heightSummary = await plants.aggregate([
        { $unwind: "$Height (feet) By Number" },
        { $group: { _id: null, max: { $max: "$Height (feet) By Number" } } },
      ]).toArray();
      const maxHeight = Math.ceil(heightSummary[0]?.max || 0);
      const heights = [];
          ...
          if (values.length) {
            if (filter.name === "Family") {
              $and.push({
                $or: [
                  { Family: { $in: values } },
                  { "Plant Family": { $in: values } },
                ],
              });
            } else if (filter.name === "Genus") {
              const scientificNamePrefixes = values.map(
                (value) => new RegExp(`^${quote(value)}(?:\\s|$)`, "i")
              );
              $and.push({
                $or: [
                  { Genus: { $in: values } },
                  { "Plant Genus": { $in: values } },
                  { "Scientific Name": { $in: scientificNamePrefixes } },
                ],
              });
            } else {
              $and.push({
            ...
            if (input && input.length) {
              if (filter.name === "Family") {
                matchAnd.push({
                  $or: [
                    { Family: { $in: input } },
                    { "Plant Family": { $in: input } },
                  ],
                });
              } else if (filter.name === "Genus") {
                const scientificNamePrefixes = input.map(
                  (value) => new RegExp(`^${quote(value)}(?:\\s|$)`, "i")
                );
                matchAnd.push({
                  $or: [
                    { Genus: { $in: input } },
                    { "Plant Genus": { $in: input } },
                    { "Scientific Name": { $in: scientificNamePrefixes } },
                  ],
                });
              } else {
      ...
      if (!req.query.favorites) {
        ...
        if (heightFilter && heightFilter.range) {
          ...
          if (query._useSemantic && scoredResults) {
            const allHeights = [];
            for (const plant of scoredResults) {
              const heightValues = plant["Height (feet) By Number"] || [];
              allHeights.push(...heightValues.filter((value) => typeof value === "number"));
            }
            ...
            // Use the normalized numeric height array rather than display text ranges.
            const heightAggregation = await plants.aggregate([
              ...
              { $unwind: "$Height (feet) By Number" },
              {
                $group: {
                  _id: null,
                  minHeight: { $min: "$Height (feet) By Number" },
                  maxHeight: { $max: "$Height (feet) By Number" }
                }
              }
            ]).toArray();
  ...
  app.get("/api/v1/nurseries", async (req, res) => {
    ...
    const requestedState = normalizeNurseryStateFilter(req.query.state);
    if (requestedState) {
      query.stateOrRegion = new RegExp(`^${quote(requestedState)}$`, "i");
    }
