...
module.exports = async function ({ plants, nurseries }) {
  ...
  app.get("/api/v1/plants", async (req, res) => {
    try {
      ...
      let maxHeight = 0;
      if (heightDoc && heightDoc["Height (feet)"] !== undefined && heightDoc["Height (feet)"] !== null) {
        const heightValue = typeof heightDoc["Height (feet)"] === 'number' 
          ? heightDoc["Height (feet)"] 
          : parseFloat(heightDoc["Height (feet)"]);
        if (!isNaN(heightValue)) {
          maxHeight = Math.ceil(heightValue);
        }
      }
      const heights = [];
      ...
      let maxSpread = 0;
      if (spreadDoc && spreadDoc["Spread (feet)"]) {
        const spreadValue = parseFloat(spreadDoc["Spread (feet)"]);
        if (!isNaN(spreadValue)) {
          maxSpread = Math.floor(spreadValue);
        }
      }
      const spreads = [];
          ...
          if (query._useSemantic && scoredResults) {
            ...
            for (const plant of scoredResults) {
              ...
              if (heightValue !== undefined && heightValue !== null) {
                const num = typeof heightValue === 'number' ? heightValue : parseFloat(heightValue);
                if (!isNaN(num)) {
                  allHeights.push(num);
                }
              }
            ...
            const heightAggregation = await plants.aggregate([
              { $match: matchQuery },
              {
                $addFields: {
                  heightNum: {
                    $cond: {
                      if: { $eq: [{ $type: "$Height (feet)" }, "string"] },
                      then: { $toDouble: "$Height (feet)" },
                      else: { $toDouble: "$Height (feet)" }
                    }
                  }
                }
              },
              {
                $match: {
                  heightNum: { $exists: true, $ne: null, $not: { $eq: NaN } }
                }
              },
              {
                $group: {
          ...
          if (query._useSemantic && scoredResults) {
            ...
            for (const plant of scoredResults) {
              ...
              if (spreadValue !== undefined && spreadValue !== null) {
                const num = parseFloat(spreadValue);
                if (!isNaN(num)) {
                  allSpreads.push(Math.floor(num));
                }
              }
            }
            ...
            const spreadAggregation = await plants.aggregate([
              { $match: matchQuery },
              {
                $addFields: {
                  spreadNum: {
                    $cond: {
                      if: { $eq: [{ $type: "$Spread (feet)" }, "string"] },
                      then: { $toDouble: "$Spread (feet)" },
                      else: { $toDouble: "$Spread (feet)" }
                    }
                  }
                }
              },
              {
                $match: {
                  spreadNum: { $exists: true, $ne: null, $not: { $eq: NaN } }
                }
              },
              {
                $group: {
                  _id: null,
                  minSpread: { $min: { $floor: "$spreadNum" } },
                  maxSpread: { $max: { $floor: "$spreadNum" } }
                }
              }
            ]).toArray();
