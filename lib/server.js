...
module.exports = async function({ plants, nurseries }) {
  ...
  app.post('/get-vendors', async(req, res) =>{
    ...
    const url = `${baseUrl}/Plant/FindVendorsForPlantName?plantName=${req.body.plantName}&zipCode=${req.body.zipCode}&radius=${req.body.radius}&limit=5`;
    var response = await axios.get(url, {headers: {'Authorization': 'Bearer ' + apiKey}})
  ...
  app.post('/get-city', async(req, res)=> {
    ...
    const baseUrl = process.env.PAC_API_BASE_URL
    const apiKey = process.env.PAC_API_KEY
    const url = `${baseUrl}/vendor/FindCity?zipCode=${req.body.zipCode}`;
    var response = await axios.get(url, {headers: {'Authorization': 'Bearer ' + apiKey}})
    return res.send(response.data)
  })
  app.post('/get-zip', async(req, res)=> {
    ...
    const baseUrl = process.env.PAC_API_BASE_URL
    const apiKey = process.env.PAC_API_KEY
    const url = `${baseUrl}/vendor/FindZip?lat=${req.body.latitude}&lng=${req.body.longitude}`;
    var response = await axios.get(url, {headers: {'Authorization': 'Bearer ' + apiKey}})
    return res.send(response.data)
  })
